...
3. **M3C ChatGPT Tools (Future):** Might consume this snapshot via MCP.
...
1. **Never Interpet Without Context:** The `last_price` field must never be interpreted without considering `freshness_status`, `delay_status`, `source_used`, `source_authority`, `staleness_seconds`, `price_semantics`, and `caveats`.
...
A block defining the current operational status of the sources queried during snapshot generation. Must list sources, their connectivity status (`http_ok`), response latencies, and any high-level errors.
...
* `staleness_seconds`: An integer representing the elapsed time between `source_time` and `retrieved_time` (or `null` if unable to calculate).
...
1. **Failed Symbols:** Symbols that could not be retrieved from any source must be explicitly listed in the `failed_symbols` array, preserving error metadata. They MUST NOT be silently omitted.
2. **Failed Sources:** If a source fails globally, it MUST be noted in `failed_sources` and `source_health`.
