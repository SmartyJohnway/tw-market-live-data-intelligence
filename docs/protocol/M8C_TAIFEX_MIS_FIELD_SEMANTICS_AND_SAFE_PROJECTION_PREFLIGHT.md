# M8C TAIFEX MIS field semantics and safe projection preflight
...
## Safe projection candidates

Candidate safe fields are last, reference, best_bid, best_ask, best_bid_size, best_ask_size, volume, source date, source time, source status code, and market phase when directly observed. `CTestPrice` and `CTestVolume` remain indicative fields and must not be merged with `CLastPrice`. `CRefPrice` remains `reference`, not previous close. SettlementPrice and OpenInterest remain unresolved for active AI context until update timing is verified. Only top-of-book is eligible; full ladders and raw QID maps are blocked.
