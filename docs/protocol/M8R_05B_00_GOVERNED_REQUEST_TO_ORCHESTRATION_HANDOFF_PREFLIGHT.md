...
## Commit-3 contract precision correction

The plan identity scope is now machine-readable: it hashes all semantic plan fields, including ordered operations, batch groups, accounting, blocked records, machine-readable omitted optional-capability records, and machine-readable warning code/scope/severity/reason. Only `plan_id`, `plan_hash`, planning timestamp, warning display text, human-readable display messages, and presentation formatting are excluded. `plan_hash` is SHA-256 of those exact canonical bytes and `plan_id` is a prefix of that same digest.

Only `executable_pending_approval` records can enter executable batch groups and require non-null executor and batch identifiers. Plan-only records have a null batch identifier and no invocation or network-estimate contribution. Blocked records are held separately with non-empty blocking codes. Omitted optional capabilities are held in a dedicated canonical collection, never as operations.

`planned_evidence_bundle_count` is a non-authorizing plan estimate: one only for a future executable package attempt and zero for blocked or wholly plan-only packages. Actual evidence-bundle counts belong solely to the M8R-05B-03 execution receipt; network estimates are not execution results or retry allowances.
