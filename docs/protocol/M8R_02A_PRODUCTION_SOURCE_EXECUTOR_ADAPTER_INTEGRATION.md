...
For TAIFEX MIS, the approved target derivative identity is translated to the existing M8C selector fields: `instrument_type`, `requested_product_id`, `contract_month_or_week`, `session`, and for options `strike_price` and `option_type`. The M8C executor validates selectors, resolves runtime identity, collects a bounded initial snapshot, and the context adapter exposes returned identity through `safe_fields.contract_identity`. The M8R adapter returns only identity evidence present in that accepted runtime/context output. For options, exact identity includes underlying. Underlying is not copied from approved input; it is derived only from returned/runtime product-series evidence through the bounded MVP canonical mapping `TXO -> TX`, otherwise the operation fails with `exact_option_underlying_not_returned`. This mapping is not universal TAIFEX option-family coverage. Unsupported weekly or after-hours identities block before network.

For TAIFEX OpenAPI, the adapter passes requested product, session, month, strike, and option type to the M8B executor as `requested_identity`, but `returned_identity` is built only from accepted M8B observation evidence (`contract_identity`, `aggregate_identity`, `product_id`, or equivalent normalized fields). Option underlying is derived only from returned product-series evidence through the same bounded `TXO -> TX` mapping used by TAIFEX MIS. The adapter records `identity_level` and `identity_verification_status`; it uses `contract_level` only when all required returned fields are present and matched, uses `product_level`/`aggregate_level` for non-contract observations, and fails exact-contract requests with `exact_contract_identity_not_returned` when M8B does not return exact identity evidence.
...
Expected source exceptions become stable issue codes such as `source_timeout`, `source_connection_failed`, `source_payload_invalid`, `source_identity_mismatch`, `source_market_mismatch`, `source_instrument_type_mismatch`, `target_not_present_in_source_result`, `exact_contract_identity_not_returned`, `exact_option_underlying_not_returned`, and `exact_contract_not_supported`. Raw exception text, URLs, headers, cookies, tokens, and response bodies are not copied.
