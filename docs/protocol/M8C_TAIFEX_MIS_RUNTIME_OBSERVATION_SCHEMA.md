...
Field precedence is per field: SockJS numeric QID, exact `getQuoteDetail` named field, then quote-list named field. Required numeric mappings include `125 -> CLastPrice`, `129 -> CRefPrice`, `143 -> CTime`, `144 -> CDate`, `145 -> Status`, `404 -> CTotalVolume`, and the top-of-book candidate families `101/102/113/114` and `743/744/745/746`. QID `101` is bid-price family data only and is never used as last price.

Missing values stay null; malformed values stay null with a caveat; zero is a valid zero. Decimal is used for numeric normalization. `trueValues`, raw QID maps, cookies, SockJS session paths, full option chains, and raw payloads are never returned.
