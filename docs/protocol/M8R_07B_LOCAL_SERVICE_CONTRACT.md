...
The stable response contains: service version, control package/result IDs and hash, result status, canonical `request_id`, `request_mode`, verified receipt-derived `execution_outcome`, canonical Result, existing AI-ready Markdown, complete Audit-derived citation references, Result/Audit references, and materialization status. `request_mode` is the original request field; `execution_outcome` is the verified receipt `overall_status`, never inferred from Result status.
