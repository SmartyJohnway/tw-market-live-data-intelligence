...
Selected latest endpoints have no documented date parameter; weekend/holiday is inferred when latest `trade_date` remains prior trading day. Do not classify latest prior trading day as source_unavailable. If future historical/date endpoints are added, empty holiday responses become `empty_non_trading_day`, not source_error.
