# M8C TAIFEX MIS source and transport preflight
...
## Direct execution evidence in this PR

Compact probes reproduced the official entrypoint (`200`, 4,443 bytes), `/futures/rt/info` (`200`, `websocket=true`, `cookie_needed=true`, `origins=["*:*"]`), and a bounded XHR polling open frame (`o`). REST endpoint paths returned JSON envelopes with `RtCode`, `RtMsg`, and `RtData`; option list with an empty request returned `400`, so M8C-01 must use scoped product/month requests and keep option discovery bounded.
