...
def main():
 ...
 for surface_name, surface in [('root',reg),('active',reg.get('m8_active_consolidated_status',{})),('planning',reg.get('planning_state',{}))]:
  ...
  if surface.get('recommended_next_task')!='M8R-03E-R4-PERFORMANCE-AND-SCALABILITY-HARDENING': fail(f'{surface_name} successor mismatch')
  if surface.get('registry_successor')!='M8R-03E-R4-PERFORMANCE-AND-SCALABILITY-HARDENING': fail(f'{surface_name} registry successor mismatch')
 ...
 if health.get('recommended_next_task')!='M8R-03E-R4-PERFORMANCE-AND-SCALABILITY-HARDENING': fail('health successor mismatch')
 if health.get('registry_successor')!='M8R-03E-R4-PERFORMANCE-AND-SCALABILITY-HARDENING': fail('health registry successor mismatch')
