...
def main():
 ...
 if c.get('recommended_next_task')!='M8R-03E-EOD-EXPECTED-TRADE-DATE-AND-NATURAL-DISASTER-SESSION-STATUS': fail('successor is not R5A')
 ...
 for surface_name, surface in [('root',reg),('active',reg.get('m8_active_consolidated_status',{})),('planning',reg.get('planning_state',{}))]:
  if surface.get('implemented_through_track')!='M8R-03E-R3-ARCHITECTURE-AND-CODE-HEALTH-CLEANUP': fail(f'{surface_name} implemented track mismatch')
  ...
  if surface.get('recommended_next_task')!='M8R-03E-EOD-EXPECTED-TRADE-DATE-AND-NATURAL-DISASTER-SESSION-STATUS': fail(f'{surface_name} successor mismatch')
  if surface.get('registry_successor')!='M8R-03E-EOD-EXPECTED-TRADE-DATE-AND-NATURAL-DISASTER-SESSION-STATUS': fail(f'{surface_name} registry successor mismatch')
 health=json.load(open(ROOT/'docs/data_capabilities/m8_repository_health_status.json'))
 if health.get('implemented_through_track')!='M8R-03E-R3-ARCHITECTURE-AND-CODE-HEALTH-CLEANUP': fail('health implemented track mismatch')
 ...
 if health.get('recommended_next_task')!='M8R-03E-EOD-EXPECTED-TRADE-DATE-AND-NATURAL-DISASTER-SESSION-STATUS': fail('health successor mismatch')
 if health.get('registry_successor')!='M8R-03E-EOD-EXPECTED-TRADE-DATE-AND-NATURAL-DISASTER-SESSION-STATUS': fail('health registry successor mismatch')
