...
def validate(path=AUTH):
    a=load(path); errs=[]
    checks=[('merge_sha','11711eacf7795f3074350a0db68b3227b7986215'),('m5c_request_sha256',sha(REQ)),('source_manifest_sha256',sha(RUN/'sha256_manifest.json')),('staging_candidate_sha256',sha(RUN/'staging_candidate.json')),('source_run_dir',str(RUN)),('destination',DEST)]
    for k,v in checks:
    ...
    required_true=['single_use','staging_promotion_authorized']
    required_false=['production_write','frontend_public_write','frontend_publication_authorized','generated_artifact_write','trading_output','realtime_claim_authorized','network_probe_authorized','actual_frontend_publication_authorized','production_ready','current_realtime','realtime_guaranteed']
