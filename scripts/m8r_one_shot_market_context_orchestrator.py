...
def normalize_operation_result(raw: Any, op: dict[str, Any], operation_id: str, *, started_at_utc: str, finished_at_utc: str) -> dict[str, Any]:
    ...
    return {"schema_version": OPERATION_RESULT_SCHEMA_VERSION, "operation_id": operation_id, "target_id": op.get("target_id"), "context_type": op.get("context_type"), "source_family": op.get("source_family"), "operation_class": op.get("operation_class"), "route": op.get("route"), "status": status, "started_at_utc": started_at_utc, "finished_at_utc": finished_at_utc, "network_attempted": bool(raw.get("network_attempted", op.get("network_required", False) and status != "blocked")), "network_request_count": int(raw.get("network_request_count", 1 if raw.get("network_attempted", op.get("network_required", False) and status != "blocked") else 0)), "adapter_invocation_count": int(raw.get("adapter_invocation_count", 1 if status != "blocked" else 0)), "source_observation": obs if isinstance(obs, dict) else {}, "source_health": raw.get("source_health") or {}, "currentness": raw.get("currentness") or (obs or {}).get("currentness") or {}, "returned_identity": raw.get("returned_identity") or (obs or {}).get("safe_fields", {}).get("contract_identity") or {}, "issues": raw.get("issues") or [], "retained_artifacts": raw.get("retained_artifacts") or [], "grouping": raw.get("grouping") or {"grouped": False}}
