...
def normalize_taifex_block_trade(*, requested_products, requested_contract_months=None, requested_strikes=None, requested_option_types=None, requested_sessions=None, retrieved_at=None, fetcher=None):
    ...
    rows=data if isinstance(data,list) else data.get("rows",[]); res["row_count_received"]=len(rows); seen=set(); dates=set()
    for i,row in enumerate(rows):
        ...
        if not(td and cm and row.get("Contract") and vol is not None and high and low and (is_future or (strike and opt))): res["row_count_rejected"]+=1; res["rejected_rows"].append({"index":i,"reason":"identity_parse_failure"}); continue
