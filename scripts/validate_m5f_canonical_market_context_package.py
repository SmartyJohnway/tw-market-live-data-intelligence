...
from build_m5f_canonical_market_context_package import build_package, reject_forbidden_nested, SYMBOL_ALLOWLIST
...
def _canonical_checks(c):
    ...
    for s in symbols:
        ...
        if not isinstance(s.get('price_like_value'), (int,float)): raise ValueError('price_like_value must be numeric')
