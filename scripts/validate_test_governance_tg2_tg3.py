...
def validate() -> dict:
    ...
    default_paths = profiles["default-ci"]["pytest_paths"]
    tg1_paths = [item["path"] for item in tg1["records"]]
    assert len(default_paths) == 156
    assert default_paths == tg1_paths
    ...
    assert set(flattened) == set(default_paths)
    ...
    return {
        "status": "PASS",
        "default_ci_file_count": len(default_paths),
