...
def normalize_taifex_options_eod(*, requested_products, requested_contract_months=None, requested_strikes=None, requested_option_types=None, requested_sessions=None, retrieved_at=None, fetcher=None):
    ...
    for i,row in enumerate(rows):
        ...
        ok_schema, missing_schema = validate_required_fields(row, ["Date","Contract","ContractMonth(Week)","StrikePrice","CallPut"])
        ...
        required_numeric=["Close","SettlementPrice","Volume","OpenInterest"]
        bad_fields=[k for k in required_numeric if (fv.get(k) or {}).get("valid") is False]
        status="partial" if bad_fields else "complete"
        if bad_fields: caveats.append("malformed_or_missing_core_fields:"+",".join(bad_fields))
        obs=create_observation(endpoint_contract_id=ENDPOINT, context_type=CONTEXT_TYPES["options"], instrument_type="options", product_id=row.get("Contract"), contract_identity={"trade_date":td,"product_id":row.get("Contract"),"contract_month_or_week":cm,"strike_price":strike,"option_type":opt,"session":session}, trade_date=td, retrieved_at_utc=retrieved_at, session=session, source_session_label=row.get("TradingSession"), observation_status=status, field_validation=fv, source_fields_present=present, omitted_source_fields=omitted, caveats=caveats, provenance={"endpoint":ENDPOINT,"raw_payload_retained":False}, payload={"price":price,"activity":activity,"open_interest":oi,"trading_halt":row.get("TradingHalt")})
        res["observations"].append(obs)
    res["batch_status"]="schema_drift" if rows and schema_valid_rows == 0 else ("date_mismatch" if len(dates)>1 else ("successful_derivatives_eod_batch" if res["observations"] else ("no_matching_bounded_scope" if rows else "empty_non_trading_day")))
