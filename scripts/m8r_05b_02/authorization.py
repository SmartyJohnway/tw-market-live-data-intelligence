...
def _derived(plan, mode, requested):
 ...
 bindings=[{'operation_id':x['operation_id'],'market':x.get('market'),'security_type':sorted(x.get('security_types',[])),'capability_id':x.get('capability_id'),'executor_id':x.get('executor_id'),'expected_evidence_contract':x.get('expected_evidence_contract'),'batch_group_id':x.get('batch_group_id')} for x in sorted(chosen,key=lambda x:x['operation_id'])]
