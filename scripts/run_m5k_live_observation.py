...
from scripts.m5k_common import DEFAULT_WATCHLIST_PATH, dump_json, execute_live_observation, load_json
...
def main() -> int:
    ...
    args = ap.parse_args()
    if not args.execute_live_observation:
        print(dump_json({"status": "check_only", "network_calls": False, "artifact_writes": False, "execute_mode_available": True}))
        return 0
    result = execute_live_observation(load_json(Path(args.watchlist)), write_latest=not args.no_write_latest)
