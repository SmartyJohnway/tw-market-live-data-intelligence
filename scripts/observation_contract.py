...
M7D_BOUNDED_WATCHLIST_CROSS_CONTEXT_SCHEMA_VERSION = "m7d_bounded_watchlist_cross_context.v1"


def _m7d_coverage_catalog() -> list[dict[str, object]]:
    return [
        {"id":"bounded_observed_market_snapshot","name":"Bounded observed market snapshot","scope_status":"in_scope_m7d","description":"Summary of the currently observed watchlist payload only.","allowed_interpretation":"Describes only items in the configured watchlist/latest observation payload.","forbidden_interpretation":"Must not imply full market coverage or market-wide direction.","depends_on":["latest_observation_payload"],"m7d_schema_group":"watchlist_observation_coverage"},
        {"id":"current_price_state","name":"Current price state","scope_status":"referenced_dependency","description":"Latest observed price state per watchlist item from M7A/M7B/M7C dependencies.","allowed_interpretation":"M7D may aggregate availability but does not redefine price semantics.","forbidden_interpretation":"Must not reinterpret reference values as current market data.","depends_on":["M7A","M7B","M7C"],"m7d_schema_group":"watchlist_observation_coverage"},
        {"id":"session_position","name":"Session position","scope_status":"deferred_m7e","description":"Whether the observation is preopen/intraday/closed/stale/session-relative.","allowed_interpretation":"Deferred until a market clock/session-state policy exists.","forbidden_interpretation":"Must not invent session clock semantics in M7D-00/01.","depends_on":["M7E_market_clock"],"m7d_schema_group":"quality_gate_policy"},
        {"id":"market_index_context","name":"Market index context","scope_status":"in_scope_m7d","description":"Bounded relationship to index-like watchlist items such as TAIEX if present.","allowed_interpretation":"Compare only against an index-like item present in the same watchlist with observations/metrics.","forbidden_interpretation":"Must not infer market prediction or full Taiwan market direction.","depends_on":["watchlist_config","M7C"],"m7d_schema_group":"index_relative_context"},
        {"id":"futures_context","name":"Futures context","scope_status":"in_scope_m7d","description":"Bounded relationship to TAIFEX/TX-like watchlist items if present.","allowed_interpretation":"Compare only against futures-like items present in the same watchlist.","forbidden_interpretation":"Must not infer basis, fair value, or futures-led direction unless explicitly supported by later policy.","depends_on":["watchlist_config","M7C"],"m7d_schema_group":"futures_relative_context"},
        {"id":"watchlist_breadth","name":"Watchlist breadth","scope_status":"in_scope_m7d","description":"Positive/negative/flat/degraded/failed/reference-only counts within current watchlist.","allowed_interpretation":"Bounded watchlist breadth only.","forbidden_interpretation":"Must not be called market breadth or generalized to full market breadth.","depends_on":["M7C"],"m7d_schema_group":"bounded_breadth_summary"},
        {"id":"cross_instrument_differences","name":"Cross-instrument differences","scope_status":"in_scope_m7d","description":"Factual comparison of watchlist items by deterministic metrics such as change_percent.","allowed_interpretation":"Descriptive comparison among observed watchlist items.","forbidden_interpretation":"Must not rank as a recommendation or trading instruction.","depends_on":["M7C"],"m7d_schema_group":"bounded_relative_change_summary"},
        {"id":"current_volume_observations","name":"Current volume observations","scope_status":"referenced_dependency","description":"Bounded observation of current volume fields if available.","allowed_interpretation":"M7D may summarize availability while preserving upstream caveats.","forbidden_interpretation":"Must not treat volume as capital flow or broad participation.","depends_on":["M7A","M7B","M7C"],"m7d_schema_group":"watchlist_observation_coverage"},
        {"id":"displayed_order_book_depth_snapshot","name":"Displayed order book depth snapshot","scope_status":"referenced_dependency","description":"Displayed top-5 quote/depth context from M7C metrics.","allowed_interpretation":"Displayed snapshot only.","forbidden_interpretation":"Must not infer true liquidity, full order book, support/resistance, or main force.","depends_on":["M7C"],"m7d_schema_group":"bounded_relative_change_summary"},
        {"id":"source_freshness_delay_failure","name":"Source freshness delay failure","scope_status":"in_scope_m7d","description":"Count/summarize fresh/degraded/stale/failed observations.","allowed_interpretation":"Use existing freshness/source-health semantics only.","forbidden_interpretation":"Must not invent market timing or session state.","depends_on":["latest_observation_payload"],"m7d_schema_group":"source_freshness_summary"},
        {"id":"missing_fields_context","name":"Missing fields context","scope_status":"in_scope_m7d","description":"Summarize missing required context fields across watchlist items.","allowed_interpretation":"Descriptive availability summary only.","forbidden_interpretation":"Must not convert missingness into direction or recommendation.","depends_on":["M7B","M7C"],"m7d_schema_group":"missing_context_summary"},
        {"id":"stale_observations_context","name":"Stale observations context","scope_status":"in_scope_m7d","description":"Identify stale/reference-only/degraded observations inside bounded watchlist.","allowed_interpretation":"Describes quality/freshness limitations within current watchlist.","forbidden_interpretation":"Must not call stale observations market direction.","depends_on":["latest_observation_payload"],"m7d_schema_group":"degraded_context_summary"},
        {"id":"official_eod_context","name":"Official EOD context","scope_status":"deferred_m8","description":"Official TWSE/TPEx/TAIFEX EOD reference baseline.","allowed_interpretation":"Deferred official EOD/reference context.","forbidden_interpretation":"Must not implement official EOD in M7D.","depends_on":["M8_official_eod_context"],"m7d_schema_group":"quality_gate_policy"},
        {"id":"recent_historical_context","name":"Recent historical context","scope_status":"deferred_m8","description":"5D/20D or recent historical baseline.","allowed_interpretation":"Deferred historical context.","forbidden_interpretation":"Must not implement history in M7D.","depends_on":["M8_recent_history"],"m7d_schema_group":"quality_gate_policy"},
        {"id":"source_health_caveats","name":"Source health caveats","scope_status":"referenced_dependency","description":"Existing source-health caveats reflected in bounded context.","allowed_interpretation":"Reference existing caveats without changing source-health behavior.","forbidden_interpretation":"Must not change source-health behavior in M7D-00/01.","depends_on":["source_health_summary"],"m7d_schema_group":"source_freshness_summary"},
        {"id":"data_provenance","name":"Data provenance","scope_status":"referenced_dependency","description":"Source, adapter, retrieved_at, and evidence lineage.","allowed_interpretation":"Schema includes provenance_summary placeholders.","forbidden_interpretation":"Must not fabricate lineage or retrieved_at values.","depends_on":["latest_observation_payload"],"m7d_schema_group":"provenance_summary"},
        {"id":"semantic_limitations","name":"Semantic limitations","scope_status":"in_scope_m7d","description":"Explicit limitations and forbidden interpretations.","allowed_interpretation":"Required caveats for any future bounded cross-context output.","forbidden_interpretation":"Must not omit no-go caveats from future builder output.","depends_on":[],"m7d_schema_group":"semantic_limitations"},
        {"id":"frontend_operator_presentation","name":"Frontend operator presentation","scope_status":"deferred_m7f","description":"Frontend/operator card, workbench layout, copy-to-AI rendering.","allowed_interpretation":"Deferred product surface work.","forbidden_interpretation":"Must not build frontend in M7D-00/01.","depends_on":["M7F_frontend_operator_presentation"],"m7d_schema_group":"quality_gate_policy"},
        {"id":"ai_discussion_handoff","name":"AI discussion handoff","scope_status":"in_scope_m7d","description":"AI-readable bounded cross-context package.","allowed_interpretation":"Schema only in M7D-00/01; controlled exposure deferred to M7D-04.","forbidden_interpretation":"Must not expose M7D to conversation context before controlled integration.","depends_on":["M7D-04"],"m7d_schema_group":"semantic_limitations"},
    ]


def build_empty_bounded_watchlist_cross_context() -> dict[str, object]:
    """Build the schema-only M7D bounded watchlist cross-context contract."""
    return {
        "schema_version": M7D_BOUNDED_WATCHLIST_CROSS_CONTEXT_SCHEMA_VERSION,
        "context_id": "M7D_BOUNDED_WATCHLIST_CROSS_CONTEXT",
        ...
        "not_trading_signal": True,
        "not_recommendation": True,
        "source_policy": {"depends_on": ["M7A_TWSE_MIS_RICH_OBSERVATION", "M7B_AI_SAFE_MARKET_CONTEXT", "M7C_DETERMINISTIC_METRICS"], "m7a_final_status": "pass_with_caveats", "m7b_final_status": "pass_with_caveats", "m7c_final_status": "pass_with_caveats", "source_discovery_required": False, "live_probe_required": False, "full_market_claim_allowed": False, "semantic_status": "schema_only"},
        "input_requirements": {"required_inputs": ["watchlist_config", "latest_observation_payload", "m7b_ai_safe_market_context_projection_collection", "m7c_deterministic_metrics_collection"], "optional_inputs": ["source_health_summary", "watchlist_group_metadata"], "forbidden_inputs": ["raw_twse_mis_payload", "raw_full_ladder_arrays", "unbounded_market_universe"], "semantic_status": "schema_only"},
        "coverage_catalog": _m7d_coverage_catalog(),
        "grouping_policy": {"allowed_grouping_dimensions": ["market", "source", "adapter_id", "instrument_type", "category", "watchlist_group", "observation_status", "freshness_status", "reference_only_status", "has_m7b_projection", "has_m7c_metrics"], "deferred_grouping_dimensions": ["industry", "sector", "capital_flow", "market_wide_breadth"], "semantic_status": "schema_only"},
        "watchlist_observation_coverage": {"watchlist_item_count": None, "observed_item_count": None, "missing_observation_count": None, "degraded_observation_count": None, "failed_observation_count": None, "reference_only_count": None, "has_m7b_projection_count": None, "has_m7c_metrics_count": None, "status": "schema_only"},
        "bounded_breadth_summary": {"bounded_positive_change_count": None, "bounded_negative_change_count": None, "bounded_flat_change_count": None, "bounded_unavailable_change_count": None, "bounded_watchlist_only": True, "not_full_market_breadth": True, "status": "schema_only"},
        "bounded_relative_change_summary": {"top_positive_change_percent_items": [], "top_negative_change_percent_items": [], "comparison_metric": "change_percent", "bounded_watchlist_only": True, "not_recommendation": True, "status": "schema_only"},
        "index_relative_context": {"index_like_items": [], "index_comparison_pairs": [], "bounded_watchlist_only": True, "requires_index_item_in_watchlist": True, "not_market_prediction": True, "status": "schema_only"},
        "futures_relative_context": {"futures_like_items": [], "futures_comparison_pairs": [], "bounded_watchlist_only": True, "requires_futures_item_in_watchlist": True, "not_futures_lead_signal": True, "status": "schema_only"},
        "etf_group_context": {"etf_like_items": [], "etf_comparison_pairs": [], "bounded_watchlist_only": True, "not_sector_rotation": True, "not_capital_flow": True, "status": "schema_only"},
        "source_freshness_summary": {"fresh_item_count": None, "stale_item_count": None, "unknown_freshness_count": None, "failed_source_count": None, "bounded_watchlist_only": True, "status": "schema_only"},
        "missing_context_summary": {"missing_m7b_projection_items": [], "missing_m7c_metrics_items": [], "missing_required_observation_items": [], "bounded_watchlist_only": True, "status": "schema_only"},
        "degraded_context_summary": {"degraded_items": [], "reference_only_items": [], "blocked_items": [], "bounded_watchlist_only": True, "status": "schema_only"},
        "provenance_summary": {"source_names": [], "adapter_ids": [], "retrieved_at_values": [], "provenance_available": False, "status": "schema_only"},
        "semantic_limitations": {"bounded_watchlist_only": True, "not_full_market_breadth": True, "not_market_wide_trend": True, "not_sector_rotation": True, "not_capital_flow": True, "not_trading_signal": True, "not_recommendation": True, "not_support_resistance": True, "not_true_liquidity": True, "not_full_order_book": True, "status": "schema_only"},
        "quality_gate_policy": {"context_status_values": ["schema_only", "not_computed", "computed", "blocked_missing_watchlist", "blocked_missing_latest_observation", "blocked_missing_m7c_metrics", "blocked_insufficient_observed_items", "blocked_quality_flags", "blocked_deferred_dependency"], "minimum_observed_items_for_cross_context": 2, "requires_bounded_watchlist": True, "requires_m7c_metrics_for_change_comparisons": True, "requires_index_item_for_index_relative_context": True, "requires_futures_item_for_futures_relative_context": True, "semantic_status": "schema_only"},
