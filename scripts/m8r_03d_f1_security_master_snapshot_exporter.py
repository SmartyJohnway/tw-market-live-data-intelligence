...
SKILL_PATH=ROOT/'skills'/'tw-security-master-classifier'
SKILL_SCHEMA_DIR=SKILL_PATH/'references'/'schemas'
...
def export_verified_security_master_snapshot(*, classification_records:list[dict], lifecycle_events:list[dict], source_context:dict, generated_at_utc:str, effective_observation_date:str|None=None)->tuple[dict,dict]:
    ...
    coverage={'markets':sorted({r['classification'].get('market') for r in records if r['classification'].get('market')}),'instrument_types':sorted({r['classification'].get('instrument_type') for r in records if r['classification'].get('instrument_type')}),'record_count':len(records),'lifecycle_event_count':retained_event_count,'coverage_status':source_context.get('coverage_status','partial_manual_snapshot'),'quarantined_lifecycle_event_count':len(quarantined),'total_lifecycle_event_count':retained_event_count+len(quarantined)}
    skill_path_str=str(SKILL_PATH.relative_to(ROOT)).replace('\\','/')
    snapshot={'schema_version':SNAPSHOT_SCHEMA_VERSION,'snapshot_id':source_context.get('snapshot_id') or 'tw-vsm-'+sha256_json({'generated_at_utc':generated_at_utc,'records':records,'events':lifecycle_events})[:16],'generated_at_utc':generated_at_utc,'effective_observation_date':as_of,'source_skill':{'name':'tw-security-master-classifier','skill_version':source_context.get('skill_version') or 'source-manifest-1.1.0','skill_path':skill_path_str,'skill_contract_hash':skill_hash},'coverage':coverage,'quarantined_lifecycle_events':quarantined,'records':records}
