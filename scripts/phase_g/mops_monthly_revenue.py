...
def execute(targets: list[dict[str, Any]], market: str, *, observed_at: str, csv_fetcher, json_fetcher) -> list[dict[str, Any]]:
    ...
    except SourceFailure as exc: return [_result(t,"source_failed",None,observed_at,route,"official_csv",False,[str(exc)]) for t in targets]
    periods={_roc_date(field(row,"資料年月","reporting_period")) for row in rows}; periods.discard(None)
    report_dates={_report_date(field(row,"出表日期","source_report_date")) for row in rows}; report_dates.discard(None)
    ...
    for target in targets:
        ...
        if status=="binding_failed": out.append(_result(target,"binding_failed",None,observed_at,route,transport,fallback,["exact_company_code_binding_failed"],diagnostics,period,report_date)); continue
        if not matches: out.append(_result(target,"not_yet_available",None,observed_at,route,transport,fallback,[],[],period,report_date)); continue
        normalized=[]
        try:
        ...
        except SourceFailure as exc: out.append(_result(target,"source_failed",None,observed_at,route,transport,fallback,[str(exc)],[],period,report_date)); continue
        if len({h for _,h in normalized}) != 1: out.append(_result(target,"binding_failed",None,observed_at,route,transport,fallback,["conflicting_duplicate_normalized_source_rows"],[v for v,_ in normalized],period,report_date)); continue
        out.append(_result(target,"available",normalized[0][0],observed_at,route,transport,fallback,[],[],period,report_date))
    return out

def _result(target,status,value,observed_at,route,transport,fallback,caveats,diagnostics=None,period=None,report_date=None):
