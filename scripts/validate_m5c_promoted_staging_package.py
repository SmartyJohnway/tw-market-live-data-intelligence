...
def validate(d, allowed_consumption_statuses=None):
    ...
    for n,o in docs.items():
        if n == 'frontend_readonly_context_package.json':
            # This immutable adapter output predates top-level package binding fields; the supplemental audit binds its hash/source without rewriting it.
            continue
    ...
    if bind.get('source_id','TWSE_OpenAPI')!='TWSE_OpenAPI' or audit.get('canonical_source_id')!='TWSE_OpenAPI': errs.append({'code':'source_binding_source_id_mismatch'})
