...
def build_watchlist_ai_context_package(*, validated_request:dict, execution_plan:dict, execution_result:dict, watchlist_bundle:dict, generated_at_utc:str, context_policy:dict|None=None) -> dict:
    ...
    for pos,tid in enumerate(pkg['request']['enabled_target_order']):
        ...
        if metrics:
            m=metrics[0]; mi=_metric_index(metrics_all,m); target['performance']={'metric_id':m.get('metric_id'),'calculation_status':m.get('calculation_status'),'requested_lookback_trading_days':_get(m,'input_period','lookback_trading_days'),'end_date':m.get('as_of'),'unadjusted_price_return':m.get('value')}; target['coverage']['evidence_states']['performance']='supported' if m.get('calculation_status')=='calculated' else 'unavailable'; target['caveats'].append('unadjusted_price_return'); target['calculation_limitations'].append('adjusted_or_total_return_not_calculated')
            pkg['evidence_limitations'].append({'code':'unadjusted_completed_eod_price_return','scope':'target','target_id':tid,'reason':'performance uses upstream unadjusted completed EOD closes'})
        ...
        if pt.get('identity_status')!='resolved': target['missing_evidence'].append(_missing(tid,'identity',pt.get('identity_status') or 'identity_unresolved',None,True,'not_recoverable_in_scope',{'plan_issues':pt.get('blocking_issues',[])})); pkg['evidence_limitations'].append({'code':'current_observation_unavailable','scope':'target','target_id':tid,'reason':'target identity or execution eligibility is unresolved'})
        if not target['current_observation']: pkg['evidence_limitations'].append({'code':'current_observation_unavailable','scope':'target','target_id':tid,'reason':'current observation unavailable'})
...
def render_watchlist_ai_context_preview(package:dict)->str:
    ...
    for t in package.get('targets',[]):
        ...
        lines += ['','#### Caveats']+[f"- {c}" for c in t['caveats']] + ['','#### Calculation limitations']+[f"- {p}" for p in t['calculation_limitations']]
