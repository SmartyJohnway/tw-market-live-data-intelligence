...
def check(cond,msg,checks,details=None):
    entry={'check':msg,'status':'passed' if cond else 'failed'}
    if not cond and details is not None:
...
def main():
    ...
    try:
        ...
        if not m5e_ok:
            details = {
                "expected_status": "superseded_by_m5f",
                "actual_status": m5e.get('status'),
                "reason": "M5E check_only output does not match expected superseded state.",
                "publication_authorized": m5e.get('frontend_publication_authorized'),
                "publication_performed": m5e.get('publication_performed')
            }
        ...
        details = {
            ...
            "traceback_excerpt": "".join(traceback.format_exception(type(exc), exc, exc.__traceback__))[-1000:]
        }
    check(m5e_ok,'m5e_superseded_by_m5f',checks,details=details)
