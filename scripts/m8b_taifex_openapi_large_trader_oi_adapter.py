...
def normalize_taifex_large_trader_oi(*, endpoint, requested_products, requested_settlement_months=None, requested_option_types=None, requested_trader_types=None, retrieved_at=None, fetcher=None):
    ...
    rows=data if isinstance(data,list) else data.get("rows",[]); res["row_count_received"]=len(rows); dates=set(); isopt=endpoint.endswith("Options")
    for i,row in enumerate(rows):
        ...
        dates.add(td); present,omitted=source_field_presence(row,FIELDS_O if isopt else FIELDS_F); caveats=["large trader open-interest concentration; not three-institutional-investor positioning"]
