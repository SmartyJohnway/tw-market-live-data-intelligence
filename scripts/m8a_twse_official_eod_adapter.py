"""TWSE official latest EOD adapter for M8A."""
...
def fetch_twse_official_eod_json(*, timeout: int=20, url: str=URL) -> tuple[list[dict], int, str]:
    ...
    with urllib.request.urlopen(req, timeout=timeout) as resp:
        status=resp.status; ctype=resp.headers.get("Content-Type",""); body=resp.read()
...
def parse_twse_official_eod_rows(rows: Any, *, requested_symbols: list[str], retrieved_at_utc: str|None=None, http_status:int|None=None, security_master:dict|None=None) -> dict:
    ...
    for i,row in enumerate(rows):
        ...
        result["row_count_examined"]+=1
        trade_date,date_val=parse_roc_yyyymmdd(row.get("Date")); fv={"Date":date_val}
        price={}; activity={};
        for src,dst in [("OpeningPrice","open"),("HighestPrice","high"),("LowestPrice","low"),("ClosingPrice","close")]: price[dst],fv[src]=parse_decimal_text(row.get(src))
        price["change"],fv["Change"]=parse_decimal_text(row.get("Change"),allow_negative=True)
        for src,dst in [("TradeVolume","trade_volume"),("TradeValue","trade_value"),("Transaction","transaction_count")]: activity[dst],fv[src]=parse_int_text(row.get(src))
        derived=[]
    ...
    if len(result["reported_trade_dates"])>1: result["batch_status"]="date_mismatch"; result["caveats"].append("mixed trade dates in retained observations")
