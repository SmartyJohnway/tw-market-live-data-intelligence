...
def load_f3_verified_security_master(snapshot_path, manifest_path, *, allow_fixture_snapshot=False):
    ...
    validated = load_verified_security_master_snapshot(
        snapshot_path,
        manifest_path,
        allow_fixture_snapshot=allow_fixture_snapshot,
        # Historical deterministic fixtures retain their producer-time Skill
        # hash.  They still validate their own snapshot/manifest binding, but
        # must not be rewritten whenever the current portable Skill changes.
        require_current_skill_contract=not allow_fixture_snapshot,
    )
