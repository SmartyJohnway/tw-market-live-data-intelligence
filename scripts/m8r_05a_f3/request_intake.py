...
def _catalog_valid(catalog):
    ...
    if catalog["schema_version"] == "unified_market_evidence_capability_catalog.v3":
        versions=catalog.get("contract_versions")
        if not isinstance(versions,dict) or versions.get("accepted_request_schema_versions") != ["unified_market_evidence_request.v1", "unified_market_evidence_request.v2", "unified_market_evidence_request.v3"] or versions.get("preferred_request_schema_version") != "unified_market_evidence_request.v3" or versions.get("emitted_result_schema_version") != "unified_market_evidence_result.v3" or "future_candidate_request_schema_version" not in versions or "future_candidate_result_schema_version" not in versions or versions.get("future_candidate_request_schema_version") is not None or versions.get("future_candidate_result_schema_version") is not None or versions.get("v3_runtime_authority_status") != "v3_preferred_selected_routes_active": return False
    ids=[]
    for cap in caps:
