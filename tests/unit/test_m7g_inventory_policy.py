...
def test_m7g_inventory_status_and_boundaries():
    ...
    assert entry['status'] == 'real_safe_artifact_rendering_and_provenance_source_health_defined'
    assert entry['completed_tasks'] == ['M7G-00','M7G-01','M7G-02','M7G-03','M7G-04','M7G-05']
    ...
    for key in ['runtime_behavior_changed','fastapi_changed','mcp_changed','backend_api_changed','live_probe_added','runtime_network_fetch_added','hidden_fetch_added','auto_refresh_added','manual_refresh_added','refresh_execution_added','ai_model_call_added','db_write_added','raw_payload_exposure_allowed','raw_forbidden_values_rendered','raw_forbidden_values_copied','trading_advice_allowed','source_health_live_probe_added','rejected_artifact_reaches_renderer']:
        assert entry[key] is False
    assert entry['next_task'] == 'M7G-06-AI-HANDOFF-FROM-LOADED-SAFE-ARTIFACT'
