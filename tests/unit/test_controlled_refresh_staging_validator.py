...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda p: p["validation"].__setitem__("frontend_write", True),
        lambda p: p["validation"].__setitem__("generated_artifact_write", True),
        lambda p: p["validation"].__setitem__("trading_signal", True),
        lambda p: p.__setitem__("target_universe", {"mode": "full_market"}),
        lambda p: p.__setitem__("target_universe", {"mode": "all"}),
        lambda p: p.__setitem__("target_universe", {"mode": "*"}),
        lambda p: p.__setitem__("target_universe", {"mode": "Full_Market"}),
        lambda p: p.__setitem__("target_universe", {"mode": "ALL"}),
        lambda p: p.__setitem__("target_universe", {"scope": "Full_Market"}),
    ],
)
def test_invalid_payload_mutations_fail(mutate):
    p = payload()
    mutate(p)
    assert errs(p)
...
def test_invalid_payload_builder_cases_record_validation_errors(kwargs):
...
def test_allowed_target_universe_cases_pass(target_universe):
    p = payload()
