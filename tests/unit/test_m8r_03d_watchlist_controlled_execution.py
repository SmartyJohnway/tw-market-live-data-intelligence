...
def test_plan_security_master_routes_blocking_and_target_limit():
    ...
    lifecycle_req=json.loads(json.dumps(req)); lifecycle_req['persistent_watchlist_reference']['enabled_target_ids']=['TWSE:2222','TWSE:1111','TWSE:3333']
    lp=build_execution_plan(lifecycle_req,bundle_type='snapshot',security_master=SM); by_life={t['target_id']:t for t in lp['targets']}
    assert by_life['TWSE:2222']['lifecycle_state']=='unknown' and by_life['TWSE:2222']['lifecycle_resolution_status']=='unavailable'
    assert by_life['TWSE:2222']['execution_policy']=='execution_allowed_with_caveat' and by_life['TWSE:2222']['current_source_plan']
    assert by_life['TWSE:1111']['identity_status']=='lifecycle_unsupported'
    assert by_life['TWSE:3333']['identity_status']=='identity_conflict' and not by_life['TWSE:3333']['current_source_plan']
    ok_dup=json.loads(json.dumps(req)); ok_dup['persistent_watchlist_reference']['enabled_target_ids']=['TWSE:3333']
    sm_ok=dict(SM); sm_ok[('listed','3333')]={**SM[('listed','3333')], 'cross_market_duplicate_policy':'exact_requested_market_ok'}
    assert build_execution_plan(ok_dup,bundle_type='snapshot',security_master=sm_ok)['targets'][0]['identity_status']=='resolved'
...
def test_fixture_snapshot_performance_and_partial(tmp_path, monkeypatch):
    monkeypatch.setattr(executor,'AUTHORIZATION_CONSUMPTION_ROOT',tmp_path/'receipts')
    req=load(C/'snapshot_request.json'); pre=execute_watchlist(req,mode='preflight',bundle_type='snapshot',artifact_root=str(tmp_path),generated_at_utc='2026-07-16T01:30:05Z',security_master=SM)
    assert pre['status']=='success' and not (tmp_path/'receipts').exists()
    out=execute_watchlist(req,mode='fixture',bundle_type='snapshot',fixture_source_data=load(FIX/'snapshot_source_data.json'),artifact_root=str(tmp_path),generated_at_utc='2026-07-16T01:30:05Z',security_master=SM)
...
def test_successful_injected_execute_and_replay(tmp_path, monkeypatch):
    monkeypatch.setattr(executor,'AUTHORIZATION_CONSUMPTION_ROOT',tmp_path/'receipts')
    ...
    replay=execute_watchlist(req,mode='execute',bundle_type='snapshot',authorization=a,artifact_root=str(tmp_path/'other_root'),generated_at_utc='2026-07-16T01:31:05Z',executors={'TWSE_MIS':fake('TWSE_MIS')},security_master=SM)
...
def test_source_failure_isolation_all_failure_and_artifact_failure(tmp_path, monkeypatch):
    ...
    assert out['status']=='success_with_partial_coverage'
    assert any(g['status']=='failed' for g in out['source_execution_summary']['group_results'])
...
def test_execute_authorization_rejections_and_cli(tmp_path, monkeypatch):
    monkeypatch.setattr(executor,'AUTHORIZATION_CONSUMPTION_ROOT',tmp_path/'receipts')
    req=load(FIX/'mixed_snapshot_request.json'); plan=build_execution_plan(req,bundle_type='snapshot',security_master=SM)
