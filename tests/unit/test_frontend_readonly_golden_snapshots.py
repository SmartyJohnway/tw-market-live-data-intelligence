...
def test_builder_output_matches_golden():
    ...
    for s,g in pairs:
        assert build_frontend_readonly_context_package(json.loads((ROOT/'tests/fixtures/staging_payloads'/s).read_text(encoding="utf-8"))) == json.loads((ROOT/'tests/fixtures/frontend_readonly_packages'/g).read_text(encoding="utf-8"))
def test_caveats_and_forbidden_flags():
    for p in (ROOT/'tests/fixtures/frontend_readonly_packages').glob('golden_*.json'):
        data=json.loads(p.read_text(encoding="utf-8")); assert all(c in data['global_caveats'] for c in REQUIRED_CAVEATS)
