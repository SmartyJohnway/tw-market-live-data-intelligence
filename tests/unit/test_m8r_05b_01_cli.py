...
def files(tmp):
 ...
 for k,x in {'v':v,'c':CAT,'r':ROUTE,'h':HAND,'i':INV,'b':bindings(v)}.items():
  p=tmp/(k+'.json');p.write_text(json.dumps(x), encoding="utf-8");out[k]=p
 return out
...
def test_success_schema_and_check_only_preserves(tmp_path,capsys):
 ...
 out=tmp_path/'plan.json';assert main(argv(p,['--output-root',str(tmp_path),'--output-relative','plan.json']))==0;v=json.loads(out.read_text(encoding="utf-8"));jsonschema.Draft7Validator(SCHEMA,format_checker=jsonschema.FormatChecker()).validate(v);assert v['execution_authorized'] is False
...
def test_planning_failure_preserves_output(tmp_path,capsys):
 p=files(tmp_path);out=tmp_path/'plan';out.write_bytes(b'old');p['b'].write_text(json.dumps({**bindings(validation()),'capability_catalog_hash':'0'*64}), encoding="utf-8")
...
def test_symlink_and_directory_rejected(tmp_path,capsys):
 p=files(tmp_path);outside=tmp_path/'outside';outside.write_text('x', encoding="utf-8");link=tmp_path/'link'
 try: link.symlink_to(outside)
 except OSError: pytest.skip('symlink unavailable')
 assert main(argv(p,['--output-root',str(tmp_path),'--output-relative','link']))==2;assert error(capsys) in {'output_destination_symlink_forbidden','output_path_forbidden'};assert outside.read_text(encoding="utf-8")=='x'
