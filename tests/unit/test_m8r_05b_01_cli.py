...
def test_check_only_and_output(tmp_path):
 ...
 assert main(args[:-1]+['--output-root',str(tmp_path),'--output-relative','out.json'])==0
...
def test_output_contract_and_traversal_fail_closed(tmp_path):
 assert main(['--validation-input','x'])==2
 # Parser/containment failures are normalized; no plan file can be written.
 v=validation(); paths={}
 for key,value in {'v':v,'c':CAT,'r':ROUTE,'h':HAND,'i':INV,'b':bindings(v)}.items():
  p=tmp_path/(key+'.json');p.write_text(json.dumps(value));paths[key]=p
 base=['--validation-input',str(paths['v']),'--capability-catalog',str(paths['c']),'--routing-matrix',str(paths['r']),'--handoff-contract',str(paths['h']),'--executor-disposition',str(paths['i']),'--input-bindings',str(paths['b']),'--planning-timestamp','2026-07-23T00:00:00Z','--output-root',str(tmp_path),'--output-relative','../escape.json']
 assert main(base)==2 and not (tmp_path.parent/'escape.json').exists()
