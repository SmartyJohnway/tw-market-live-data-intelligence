...
def test_registry_policies_remain_safe():
    ...
    assert _source("TAIFEX_MIS")["ai_context_allowed"] is True
    assert _source("TAIFEX_MIS")["ai_exposure_level"] == "controlled_caveated_safe_fields"
...
def test_broad_current_profile_includes_m8_00_04_test():
    from scripts.test_governance_authority import profile_paths_for_role
    assert "tests/unit/test_m8_source_freshness_evaluator.py" in profile_paths_for_role("broad_current")
