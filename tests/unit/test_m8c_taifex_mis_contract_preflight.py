...
def test_transport_registry_preflight_not_runtime():
    d=json.loads(pathlib.Path('docs/data_capabilities/m8c_taifex_mis_transport_registry.json').read_text(encoding="utf-8"))
...
def test_option_scope_not_exact_network_scope():
    d=json.loads(pathlib.Path('docs/data_capabilities/m8c_taifex_mis_rest_endpoint_registry.json').read_text(encoding="utf-8"))
...
def test_capability_true_has_evidence_artifact():
    evidence=json.loads(pathlib.Path('research/probe_runs/m8c_taifex_mis_preflight/sockjs_initial_state_summary.json').read_text(encoding="utf-8"))
    ...
    identities=json.loads(pathlib.Path('research/probe_runs/m8c_taifex_mis_preflight/identity_resolution_summary.json').read_text(encoding="utf-8"))['identities']
...
def test_rest_scoped_evidence_not_empty_body_probe():
    d=json.loads(pathlib.Path('research/probe_runs/m8c_taifex_mis_preflight/rest_endpoint_probe_summary.json').read_text(encoding="utf-8"))
    records=d['endpoints']
...
def test_planning_state_has_single_next_task_and_inventory_section():
    reg=json.loads(pathlib.Path('docs/data_capabilities/m8_source_capability_registry.json').read_text(encoding="utf-8"))
    ...
    inv=json.loads(pathlib.Path('docs/data_capabilities/twse_mis_rich_field_inventory.json').read_text(encoding="utf-8"))['m8c_00_taifex_mis_preflight']
...
def test_final_acceptance_does_not_contain_stale_counter_evidence():
    text=pathlib.Path('docs/protocol/M8C_00_TAIFEX_MIS_PREFLIGHT_FINAL_ACCEPTANCE.md').read_text(encoding="utf-8")
