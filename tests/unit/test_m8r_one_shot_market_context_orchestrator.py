...
from scripts.m8r_one_shot_market_context_orchestrator import (
    ...
    write_execution_artifacts, EXECUTOR_REGISTRY,
)
...
def test_unknown_executor_and_research_source_fail_closed():
    ...
    assert out["execution_status"]=="blocked" and out["network_operations_attempted"]==0
...
def test_complete_success_operation_types(target, source):
    ...
    p=plan_for([target], contexts=ctx, sources=[source]); a=approve(p); f=Fake(returned_identity={"expiry":"202607","contract_type":"monthly"})
...
def test_taifex_exact_identity_passed_mismatch_and_unsupported_blocked_before_network():
    p=plan_for([future()], sources=["TAIFEX_MIS"]); a=approve(p); good=Fake(returned_identity={"expiry":"202607","contract_type":"monthly"})
    ...
    assert out3["execution_status"]=="blocked" and out3["network_operations_attempted"]==0
