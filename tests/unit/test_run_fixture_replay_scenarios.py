...
def test_replay_checks_expected_caveats(tmp_path):
 data=json.loads((ROOT/'tests/fixtures/replay_scenarios/valid_replay_scenarios.json').read_text(encoding="utf-8")); data['scenarios'][0]['expected_frontend_caveats']=['missing_caveat']
 p=tmp_path/'scenarios.json'; p.write_text(json.dumps(data), encoding="utf-8")
...
def test_replay_summary_status_compares_actual_result(tmp_path):
 data=json.loads((ROOT/'tests/fixtures/replay_scenarios/valid_replay_scenarios.json').read_text(encoding="utf-8")); data['scenarios'][0]['expected_summary_status']='fail'
 p=tmp_path/'scenarios.json'; p.write_text(json.dumps(data), encoding="utf-8")
