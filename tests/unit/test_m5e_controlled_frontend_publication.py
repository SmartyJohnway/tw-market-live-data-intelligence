...
def test_transaction_claims_single_use(tmp_path):
    ...
    m5e.publish_transaction(src,dest,journal,auth_id='auth-claim',claim_dir=claims)
    assert (claims/'auth-claim.used').exists()
    with pytest.raises(FileExistsError): m5e.publish_transaction(src,tmp_path/'dest2',tmp_path/'j2',auth_id='auth-claim',claim_dir=claims)
