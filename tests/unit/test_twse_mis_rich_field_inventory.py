...
def test_m7a02_rich_observation_contract_inventory_registration():
    inv = load_inventory()
    ...
    assert contract["attach_helper_runtime_called"] is False
    assert inv["probe_executed"] is True
    assert inv["probe_evidence_available"] is True


def test_m_and_nu_remain_unknown_preserve_raw_only_not_safe():
    inv = load_inventory()
    fields = {row["raw_field"]: row for row in inv["field_inventory"]}
    for raw_field in ["m", "nu"]:
        row = fields[raw_field]
        assert row["observed_in_probe"] is True
        assert row["semantic_status"] == "unknown"
        assert row["normalization_status"] == "preserve_raw_only"
        assert row["ai_exposure_status"] == "not_safe_yet"
        assert row["unit_status"] == "unverified"
