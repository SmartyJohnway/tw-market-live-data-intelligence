...
def test_registry_active_state_consolidated():
    ...
    assert reg['status']=='m8c_02_code_staged_pending_remote_validation'
    ...
    assert active['next_task']==expected
    assert active['next_task_status']=='pending_remote_validation'
...
def test_docs_and_boundaries():
    ...
    assert 'TAIFEX_MIS` has staged M8C-02 controlled M8 context code pending remote validation' in readme
