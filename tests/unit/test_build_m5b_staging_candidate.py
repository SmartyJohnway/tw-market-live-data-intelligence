...
def write_minimal_run(run_dir: Path, result: dict):
    ...
    (run_dir / 'run_summary.json').write_text(json.dumps({'run_id': result['run_id'], 'staging_candidate_created': False}), encoding="utf-8")
...
def test_build_staging_candidate_finalizes_manifest_and_ledger(tmp_path):
    ...
    ledger = json.loads((tmp_path / 'evidence_ledger.json').read_text(encoding="utf-8"))
    manifest = json.loads((tmp_path / 'sha256_manifest.json').read_text(encoding="utf-8"))
...
def test_manifest_verifier_detects_tamper_and_missing_artifact(tmp_path, case):
    ...
    build(tmp_path)
    if case == "tamper":
        data = json.loads((tmp_path / "bounded_probe_result.json").read_text(encoding="utf-8"))
        data["retained_targets"] = ["2330", "2317"]
        (tmp_path / "bounded_probe_result.json").write_text(json.dumps(data), encoding="utf-8")
