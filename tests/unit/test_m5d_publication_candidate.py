...
def test_missing_caveat(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'market-context.json'; x = json.loads(p.read_text(encoding="utf-8")); x['global_caveats'] = []; write_json(p, x)
    assert any('missing_caveat' in e for e in validate_candidate(d))

def test_temporary_path_leakage(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'candidate_summary.json'; p.write_text(p.read_text(encoding="utf-8").replace('TWSE_OpenAPI', '/tmp/leak'))
    assert 'temporary_path_leakage' in validate_candidate(d)

def test_forbidden_flags_all_artifacts(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'publication_plan.json'; x = json.loads(p.read_text(encoding="utf-8")); x['trading_signal'] = True; write_json(p, x)
...
def test_manifest_final_false_fails_candidate_validator(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'sha256_manifest.json'; x = json.loads(p.read_text(encoding="utf-8")); x['manifest_final'] = False; write_json(p, x)
...
def test_upstream_binding_mutation_fails_candidate_validator(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'sha256_manifest.json'; x = json.loads(p.read_text(encoding="utf-8")); x['pr57_merge_sha'] = '0' * 40; write_json(p, x)
...
def test_source_package_changed_detection(tmp_path):
    d = copy_candidate(tmp_path); p = d / 'sha256_manifest.json'; x = json.loads(p.read_text(encoding="utf-8")); x['m5c_frontend_readonly_context_package_sha256'] = '0' * 64; write_json(p, x)
