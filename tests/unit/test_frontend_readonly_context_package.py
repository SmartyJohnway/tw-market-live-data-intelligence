...
def test_valid_staging_payload_builds_readonly_package(): assert build_frontend_readonly_context_package(payload())["readonly_only"] is True
...
def test_flags_false():
    p=build_frontend_readonly_context_package(payload()); assert p["frontend_public_artifact"] is False and p["production_current_state"] is False and p["trading_signal"] is False
