...
def test_untracked_json_artifact_blocked(tmp_path):
    d=copy_run(tmp_path); (d/'extra.json').write_text('{}', encoding="utf-8"); codes={e['code'] for e in verify_evidence(d)['errors']}; assert 'manifest_untracked_artifact' in codes
...
def test_malformed_json_returns_structured_blocked(tmp_path):
    d=copy_run(tmp_path); (d/'staging_candidate.json').write_text('{bad', encoding="utf-8")
    codes={e['code'] for e in verify_evidence(d)['errors']}
    assert 'json_read_failed' in codes
    req=tmp_path/'bad_request.json'; req.write_text('{bad', encoding="utf-8")
...
def test_blocked_cli_returns_nonzero(tmp_path):
    ...
    r=subprocess.run([sys.executable,'scripts/plan_m5c_staging_promotion.py','--destination','production/out.json','--check-only'],capture_output=True,text=True, encoding="utf-8", errors="strict"); assert r.returncode != 0
