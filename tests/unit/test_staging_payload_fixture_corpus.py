...
def test_all_valid_fixtures_validate():
    for p in valid_payloads(): assert validate_controlled_refresh_staging_payload(json.loads(p.read_text(encoding="utf-8"))) == []
def test_allowed_sources_flags_no_trading_realtime_or_production_claims():
    for p in valid_payloads():
        data=json.loads(p.read_text(encoding="utf-8")); assert data['validation']['trading_signal'] is False
        text=p.read_text(encoding="utf-8").lower(); assert 'realtime_guaranteed' not in text and 'production_current_state' not in text
        for r in data['source_runs']: assert r['source_id'] in ALLOWED_SOURCE_IDS
def test_twse_mis_risk_preserved():
    for p in valid_payloads():
        for r in json.loads(p.read_text(encoding="utf-8"))['source_runs']:
