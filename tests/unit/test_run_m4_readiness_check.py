...
def test_readiness_fails_bad_replay_scenario(tmp_path):
 bad=tmp_path/'bad_scenarios.json'; bad.write_text(json.dumps({'scenarios':[{'scenario_id':'bad','description':'bad','payload':{},'expected_validation_status':'valid','expected_frontend_caveats':[],'expected_forbidden_flags':[],'expected_summary_status':'pass','forbidden_behaviors_absent':True}]}), encoding="utf-8")
...
def test_evidence_ledger_detects_hash_mismatch(tmp_path):
 ...
 p=tmp_path/'ledger.json'; p.write_text(json.dumps(ledger), encoding="utf-8")
 assert validate_evidence_ledger(ROOT, p)[0]['code']=='hash_mismatch'
def test_release_gate_blocks_elevation(tmp_path):
 p=tmp_path/'release.json'; p.write_text(json.dumps({'current_allowed_level':'production','gates':[]}), encoding="utf-8")
...
def test_evidence_ledger_empty_or_missing_fails(tmp_path):
 p=tmp_path/'empty_ledger.json'; p.write_text(json.dumps({'evidence': []}), encoding="utf-8")
 assert validate_evidence_ledger(ROOT, p)[0]['code']=='evidence_empty'
 p2=tmp_path/'missing_ledger.json'; p2.write_text(json.dumps({}), encoding="utf-8")
...
def test_evidence_ledger_malformed_entries_structured_no_traceback(tmp_path):
 for bad_entry in ['bad', ['bad'], None]:
  p=tmp_path/'malformed_ledger.json'; p.write_text(json.dumps({'evidence': [bad_entry]}), encoding="utf-8")
...
def test_evidence_ledger_schema_failures_are_structured(tmp_path):
 ...
 p=tmp_path/'bad_ledger.json'; p.write_text(json.dumps({'evidence': [bad]}), encoding="utf-8")
