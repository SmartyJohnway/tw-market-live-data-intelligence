...
def _package():
    req,plan,res,bundle=[json.loads((FIX/name).read_text()) for name in ('request.json','execution_plan.json','execution_result.json','bundle.json')]
...
def test_agent_policy_change_does_not_change_evidence_hash_or_lineage():
    policy=json.loads((ROOT/'config/agent_policy/market_evidence_agent_policy.json').read_text()); a=_package(); policy['conversation_policy']['recommendations_permitted']=True; b=_package()
