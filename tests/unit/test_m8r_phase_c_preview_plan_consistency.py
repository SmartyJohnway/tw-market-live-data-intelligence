...
def test_preview_plan_consistency(tmp_path):
    req = make_req("test-req-consistency", ["TWSE:2330"])
    ...
    tampered_preview = json_copy(preview)
    tampered_preview["planned_operations"] = [
        op for op in tampered_preview["planned_operations"] if op["source_family"] != "TWSE_MIS"
    ]
    tampered_approval = valid_approval.copy()
    tampered_approval["preview_id"] = tampered_preview["preview_id"]
    
    res = execute_watchlist(req, mode="execute", bundle_type="snapshot", artifact_root=str(tmp_path/'other'), preview=tampered_preview, approval=tampered_approval, executors={"TWSE_MIS": dummy_mis, "TWSE_OPENAPI": dummy_eod}, source_capability_registry=PHASE_C_REGISTRY)
