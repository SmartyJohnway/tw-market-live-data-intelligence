...
def test_frontend_manual_load_ui_exists():
    for text in ['Local Safe Context Artifact Load','Manual local artifact load','Paste safe JSON artifact','Validate artifact','Load validated artifact','Reset to static demo','Validation result','Loaded artifact manifest','Loaded artifact state','Artifact provenance','Artifact currentness','Source health','Observation summary','Rejection summary']:
        assert text in HTML
...
def test_frontend_manual_action_only_and_no_forbidden_runtime_api_in_m7g_slice():
    ...
    assert 'renderM7FRichFactBrowser(m7gActiveSafeArtifact)' in SLICE
...
def test_active_context_reset_and_source_health_fallback_present():
    assert 'm7gActiveContextMode = M7G_CONTEXT_MODE_STATIC_DEMO' in SLICE
    assert 'm7gActiveSafeArtifact = null' in SLICE
    assert 'renderM7FRichFactBrowser(M7F_DEMO_RICH_CONTEXT)' in SLICE
    assert 'source_health' in SLICE
    assert 'missing optional source_health metadata in safe artifact' in SLICE
