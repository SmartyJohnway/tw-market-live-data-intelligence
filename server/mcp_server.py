...
M5N_CONVERSATION_CONTEXT_TOOL = "get_conversation_context"
...
M5Q_SOURCE_HEALTH_LATEST_TOOL = "get_source_health_latest"
M5Q_SOURCE_HEALTH_SCHEMA_TOOL = "get_source_health_schema"
...
def run_m5k_live_observation_tool(arguments: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if args.get("confirm_live_observation") is not True:
        return {"tool": M5K_LIVE_OBSERVATION_TOOL, "status": "failed_closed", "failure_reason": "missing_explicit_confirmation", "network_calls": False, "artifact_writes": False, "governance": readonly_governance() | {"layer": "M5K", "canonical": False}}
...
def get_source_health_latest_tool() -> dict[str, Any]:
    return {"tool": M5Q_SOURCE_HEALTH_LATEST_TOOL, **_m5q_read_latest_source_health()}


def get_source_health_schema_tool() -> dict[str, Any]:
    return {"tool": M5Q_SOURCE_HEALTH_SCHEMA_TOOL, "status": "ok", "content": _m5q_source_health_schema(), "governance": readonly_governance() | {"layer": "M5Q", "network_calls": False, "artifact_writes": False}}


def get_conversation_context_tool() -> dict[str, Any]:
    watchlist = _m5n_normalize_watchlist(_m5k_load_json(M5K_DEFAULT_WATCHLIST_PATH))
    return {"tool": M5N_CONVERSATION_CONTEXT_TOOL, "status": "ok", "content": _m5n_build_conversation_context(watchlist, _m5k_read_latest_observation()), "governance": readonly_governance() | {"layer": "M5N", "network_calls": False, "artifact_writes": False, "canonical": False}}
...
def create_m5k_handoff_tool(arguments: dict[str, Any] | None) -> dict[str, Any]:
    ...
    watchlist = args.get("watchlist") if isinstance(args, dict) else None
    if not isinstance(watchlist, dict):
        return {"tool": M5K_HANDOFF_TOOL, "status": "failed_closed", "failure_reason": "watchlist_required", "governance": readonly_governance() | {"layer": "M5K", "canonical": False}}
    return {"tool": M5K_HANDOFF_TOOL, "status": "ok", "content": _m5k_conversation_handoff(watchlist)}
...
async def list_tools() -> list[Tool]:
    ...
    m5k_tools = [
        ...
        Tool(name=M5N_CONVERSATION_CONTEXT_TOOL, description="Read the repaired single M5N Conversation Context package without network calls or writes.", inputSchema={"type":"object","properties":{},"additionalProperties":False}),
    ...
    m5q_tools = [
        ...
        Tool(name=M5Q_SOURCE_HEALTH_SCHEMA_TOOL, description="Read the M5Q source-health report schema without network calls or writes.", inputSchema={"type":"object","properties":{},"additionalProperties":False}),
    ]
    return [*readonly_tools, evidence_readback_tool, readiness_tool, *m5l_tools, *m5k_tools, *m5q_tools]
...
async def call_tool(name: str, arguments: dict[str, Any] | None) -> list[TextContent]:
    ...
    if name == M5Q_SOURCE_HEALTH_LATEST_TOOL:
        return _json_text(get_source_health_latest_tool())
    if name == M5Q_SOURCE_HEALTH_SCHEMA_TOOL:
        return _json_text(get_source_health_schema_tool())
    ...
    if name == M5N_CONVERSATION_CONTEXT_TOOL:
        return _json_text(get_conversation_context_tool())
    ...
    if name == M5K_LIVE_OBSERVATION_TOOL:
        return _json_text(run_m5k_live_observation_tool(arguments))
